...
def compare_models(model_output_dir: Path, out_dir: Path, metric="std"):
    ...
    box_means_grouped(full_df, metric).savefig(out_dir / f"box_plot.pdf")
    compare_means_grouped(full_df, metric).savefig(out_dir / f"compare_means.pdf")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="compare_results.py",
        description="Visually compare the benchmark results accross multiple models.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="The directory containing the result directories of multiple models "
        "(as generated by 'run_benchmark.py').",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
    ...
    parser.add_argument(
        ...
        type=str,
        default="std",
        help='Metric to use for aggregating the variation scores. Defaults to "std".',
    )
    args = parser.parse_args()

    compare_models(args.results_dir, args.output_dir, metric=args.metric)
